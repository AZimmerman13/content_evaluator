...
class TextSubNet(nn.Module):
    ...
    def forward(self, x):
        ...
        _, final_states = self.rnn(x.view(batch_size,1,1000)) # .view(64,1,1000)
...
class TFN(nn.Module):
    ...
    def __init__(self, input_dims, hidden_dims, text_out, dropouts, post_fusion_dim):
        '''
        Args:
            input_dims - a length-2 tuple, contains (img_dim, text_dim)
            hidden_dims - another length-2 tuple, similar to input_dims
            text_out - int, specifying the resulting dimensions of the text subnetwork
            dropouts - a length-3 tuple, contains (img_dropout, text_dropout, post_fusion_dropout)
